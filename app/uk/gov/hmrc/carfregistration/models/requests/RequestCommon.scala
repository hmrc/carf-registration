...
import uk.gov.hmrc.carfregistration.models.UuidGen
...
object RequestCommon {
  ...
  def apply(regime: String, uuidGenerator: UuidGen, clock: Clock): RequestCommon = {
    ...
    val currentDateTime: String = ZonedDateTime
      .now(clock)
      .format(formatter)

    RequestCommon(acknowledgementReference = acknRef, receiptDate = currentDateTime, regime = regime)
  }
}
